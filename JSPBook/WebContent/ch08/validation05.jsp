...
function checkMember(){
	...
	let regExpName =/^[가-힣]*$/ ; //이름은 한글만 사용가능, ^시작, $끝
	let regExpPasswd = /^[0-9]*$/; //비밀번호는 숫자만 사용 가능
	let regExpPhone = /^\d{3}-\d{3,4}-\d{4}$/; //전화번호 형태만 가능. \d{3}:숫자3자. \d{3,4}: 숫자 3이상 4이하
	let regExpEmail = /^[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*@[0-9a-zA-Z]([-_\.]?[0-9a-zA-Z])*\.[a-zA-Z]{2,3}$/i; //이메일 형태만 가능. i:ignore(대소문자 구별 안함)
	//폼 object
	...
	if(!regExpName.test(name)){
		alert("이름은 한글만 입력해주세요!");	//smith(x),스미스(0)
		return;
	}
